...
cc-compile-kernel:
	...
	./scripts/cross-compiler/bin/i686-elf-gcc -ffreestanding -c ./src/kernel/libc/mem.c -o ./src/mem.o
	nasm ./src/kernel/kernel_ep.asm -f elf -o ./src/kernel_ep.o
	./scripts/cross-compiler/bin/i686-elf-ld -o kernel.bin -Ttext 0x1000 ./src/kernel_ep.o ./src/kernel.o ./src/io-ports.o ./src/io-text.o ./src/mem.o --oformat binary
build-image:
	cat bootloader.bin kernel.bin > ./os-image/ampereos-dev.bin
run:
	qemu-system-i386 -hda os-image/ampereos-dev.bin
clean:
	rm -rf ./src/kernel.o ./src/kernel_ep.o ./src/io-ports.o ./src/io-text.o ./src/mem.o
